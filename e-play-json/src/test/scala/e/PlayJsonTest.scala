...
class PlayJsonTest extends ESuite:
    ...
    given testDataReads: Reads[TestData] =
        Reads[TestData]:
            case j: JsObject =>
                JsSuccess(TestData((j \ "s").asOpt[String].getOrElse(""), (j \ "i").asOpt[Int].getOrElse(0)))
            case _ => JsError("Expected an object")

    given testDataWrites: Writes[TestData] =
    ...
    test("Failing to decode an E"):
        eCodec
            .decode(Json.arr())
            .assertError(error.causes(E.name("obj").message("error.expected.jsobject")))

        eCodec
            .decode(Json.obj("code" -> "foo"))
            .assertError(error.causes(E.name("obj.code").message("error.expected.jsnumber")))

        eCodec
            .decode(Json.obj("name" -> 42))
            .assertError(error.causes(E.name("obj.name").message("error.expected.jsstring")))

        eCodec
            .decode(Json.obj("message" -> 123))
            .assertError(error.causes(E.name("obj.message").message("error.expected.jsstring")))

        eCodec
            .decode(Json.obj("causes" -> "foo"))
            .assertError(error.causes(E.name("obj.causes").message("error.expected.jsarray")))

        eCodec
            .decode(Json.obj("data" -> "foo"))
            .assertError(error.causes(E.name("obj.data").message("error.expected.jsobject")))

        eCodec
            .decode(Json.obj("time" -> "foo"))
            .assertError(error.causes(E.name("obj.time").message("error.expected.jsnumber")))

        eCodec
            .decode(Json.obj("code" -> "foo", "name" -> 42))
            .assertError(
              error.causes(
                E.name("obj.code").message("error.expected.jsnumber"),
                E.name("obj.name").message("error.expected.jsstring")
              )
            )
