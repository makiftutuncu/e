...
/** A container that can either be a Failure containing an E or Success containing a value, semantically a combination
  * of Either and Try but specialized for E
  *
  * @tparam A
...
sealed trait EOr[+A]:
    ...
    def filter(
        condition: A => Boolean,
        filteredError: A => E = { a => EOr.filteredError.data("value", a) }
    ): EOr[A] =
        self match
            case Failure(_) => self
            case Success(value) =>
                if condition(value) then value.toEOr
                else filteredError(value).toEOr[A]
...
object EOr:
    ...
    /** A default E to be used when condition does not hold while filtering an EOr
      ...
      */
    val filteredError: E =
        E(name = Some("filtered"), message = Some("Condition does not hold!"))
    ...
    def fromTry[A](t: Try[A])(ifFailure: Throwable => E): EOr[A] =
        t match
            ...
            case TryFailure(tf)            => apply(ifFailure(tf))
