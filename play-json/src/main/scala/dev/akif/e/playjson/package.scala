...
package object playjson {
  implicit val decoderJsValue: Decoder[JsValue] =
    new Decoder[JsValue] {
      override def decodeOrThrow(json: JsValue): E =
        json match {
          case obj: JsObject =>
          ...
          case j =>
            throw new DecodingError(s"'$j' is not a Json object!", null)
        }
    }

  implicit val eReads: Reads[E] = Reads { json: JsValue =>
    decoderJsValue.decode(json).fold(
      de => JsError(JsPath(), de.getMessage),
      e  => JsSuccess(e)
    )
  }
  ...
  implicit val encoderJsValue: Encoder[JsValue] =
    new Encoder[JsValue] {
      override def encode(e: E): JsValue = {
        val jsons = List(
          Json.obj("code" -> e.getCode),
          if (!e.hasName)    Json.obj() else Json.obj("name"    -> e.getName),
          if (!e.hasMessage) Json.obj() else Json.obj("message" -> e.getMessage),
          if (!e.hasCause)   Json.obj() else Json.obj("cause"   -> e.getCause.getMessage),
          if (!e.hasData)    Json.obj() else Json.obj("data"    -> mapJava2Scala(e.getData))
        )
  ...
  implicit val eWrites: Writes[E] = Writes(encoderJsValue.encode)

  implicit def maybeWrites[A: Writes]: Writes[Maybe[A]] =
    Writes {
      case Left(e)  => encoderJsValue.encode(e)
