...
class MaybeSpec extends WordSpec with Matchers {
  "Using `orE` method, a Maybe" can {
    ...
    "be made from a non-empty Option" in {
      ...
      val actual   = Some("foo").orE(E.code(3))

      actual shouldBe expected
    }

    "be made from a Left Either" in {
      val expected = Left(E.message("foo"))
      val actual   = Left[String, Int]("foo").orE(m => E.message(m))
    ...
    "be made from a Right Either" in {
      ...
      val actual   = Right[String, Int](3).orE(m => E.message(m))
    ...
    "be made from a failed Try" in {
      ...
      val expected = Left(E.cause(c))
      val actual   = Failure(c).orE(e => E.cause(e))
    ...
    "be made from a successful Try" in {
      val expected = Right("foo")
      val actual   = Success("foo").orE(e => E.cause(e))
